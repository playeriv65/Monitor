...
def update_positions(head, foot, keypoints_xy, keypoints_conf, fps):
    """更新头部和脚部位置"""
    head_valid = 0 if keypoints_conf[0] is None else 1
    foot_valid_1 = 0 if keypoints_conf[15] is None else 1
    foot_valid_2 = 0 if keypoints_conf[16] is None else 1

    # 更新头部位置
    if head_valid:
        head_position = [keypoints_xy[0][0], keypoints_xy[0][1]]
    ...
    # 更新脚部位置
    if foot_valid_1:
        foot_position_1 = [keypoints_xy[15][0], keypoints_xy[15][1]]
        foot.append(foot_position_1)
    # else:
    #     foot.append(None)

    if foot_valid_2:
        foot_position_2 = [keypoints_xy[16][0], keypoints_xy[16][1]]
...
def examiner(fqueue, rqueue):
    ...
    while cap.isOpened():
        ...
        # 摔倒检测与记录
        fall_record, fall_moment, detected = detect_fall(head, foot, hf_distance, fall_moment, frames, frame, fps)
        if detected:
